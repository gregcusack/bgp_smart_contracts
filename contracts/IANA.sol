...
contract IANA {
    ...
    function prefix_addPrefix(uint32 ip, uint8 mask, uint32 newOwnerAS) public { //, uint8 sigV, bytes32 sigR, bytes32 sigS) public {
    ...
    // function get_subPrefixOfPrefix(uint prefixIndex, uint subPrefixIndex) public view returns (uint) {
    //     if prefixes[prefixIndex].
    //     return prefixes[prefixIndex].subPrefixes[subPrefixIndex];
    // }
}
